...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  royAvatarRead: (opts: { path: string }) => invoke(IPC_ROY_AVATAR_READ, opts),
  royTelegramReport: (opts: { agentId: string; text: string; files?: string[] }) => invoke(IPC_ROY_TELEGRAM_REPORT, opts),
  royFsRename: (opts: { path: string; name: string }) => invoke(IPC_ROY_FS_RENAME, opts),
  royFsDelete: (path: string) => invoke(IPC_ROY_FS_DELETE, path),
  royFsCopy: (opts: { src: string; destDir: string }) => invoke(IPC_ROY_FS_COPY, opts),
  clipboardRead: () => invoke(IPC_CLIPBOARD_READ),
  clipboardWrite: (text: string) => invoke(IPC_CLIPBOARD_WRITE, text),
  ...
  agentsRemove: (payload: { agentId: string }) =>
    invoke<{ ok: boolean; error?: string }>(IPC_AGENTS_REMOVE, payload),
