...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.SelectTrigger
    ref={ref}
    className={cn(
      'flex h-10 w-full items-center justify-between gap-2 rounded-[14px] border border-white/10 bg-white/[0.06] px-3.5 py-2 text-sm text-foreground shadow-inner placeholder:text-muted-foreground focus:outline-none focus:border-[#0A84FF]/70 focus:ring-2 focus:ring-[#0A84FF]/30 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
      className,
    )}
    {...props}
  >
...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.SelectPortal>
    <SelectPrimitive.SelectContent
      ref={ref}
      className={cn(
        'relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-2xl glass-menu text-foreground shadow-2xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
        position === 'popper' &&
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.SelectItem
    ref={ref}
    className={cn(
      'relative flex w-full cursor-default select-none items-center rounded-xl py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-white/[0.1] focus:text-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
      className,
    )}
    {...props}
  >
