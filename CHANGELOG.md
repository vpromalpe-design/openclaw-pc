...
- **Compaction reserve capped for local models**: OpenClaw reserves ~half the context window for auto-compaction, which left a 16k-window 1.5B model with only ~6k tokens of history before `Context overflow (precheck)` blocked the agent. The wizard and engine registration now write `agents.defaults.compaction.reserveTokensFloor: 3072` when unset, roughly tripling the usable history.
