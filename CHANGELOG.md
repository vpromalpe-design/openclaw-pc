...
- **OpenRouter можно настроить из «Моделей»**: панель «⚙ Настроить» у провайдера OpenRouter раньше содержала только «Модель» и «API-ключ» — этого не хватало для работы. Теперь там есть **свой Model ID** (пункт «Свой ID модели…» вместо единственного пресета OpenRouter Auto) и **Base URL** (по умолчанию `https://openrouter.ai/api/v1`, можно указать прокси). Значения сохраняются в конфиг провайдера (`baseUrl` + `api: openai-completions`) и используются при «Проверить подключение». В мастере настройки OpenRouter настраивался и раньше — теперь страница «Модели» полноценно дублирует его.

- **Мастер настройки → панель без «ручного подключения»**: если в мастере выбрана локальная модель — движок запускается сразу при завершении настройки (до старта шлюза), панель открывается с уже запущенной моделью и зелёными индикаторами.
- **CUDA из мастера теперь «прилипает»**: установка GPU/CPU-движка в мастере (или в Моделях) сохраняет режим движка — панель стартует с GPU-индикатором зелёным, переключать вручную не нужно.
- **Удаление агента**: в меню агента (кнопка ⋯ рядом с агентом в сайдбаре) добавлена кнопка «✕ Удалить агента и все чаты» — удаляет агента из конфига, его папку с чатами/профилями и перезапускает шлюз (основной агент main защищён от удаления).
- **Фикс меню агента**: меню ⋯ больше не обрезается рамкой сайдбара — оно рендерится поверх окна (portal) и при нехватке места снизу открывается вверх.
- **Мастер настройки — кнопка «Проверить подключение» переехала в нижнюю панель**: теперь она стоит слева от кнопки «Далее» (логика и состояние теста перенесены в wizard-store, результат/ошибка видны прямо в панели; на странице «Модель» остались подсказки).
- **Голосовой ввод в текстовом чате («Просто текст»)**: кнопка 🎤 у поля ввода — нажмите, говорите, нажмите ещё раз — распознанная речь вставляется в сообщение локальным whisper.cpp (без интернета). Запись останавливается по кнопке или через 15 секунд. Если распознавание не установлено — кнопка неактивна с подсказкой.
- **STT падал при распознавании (whisper-cli exited with code 3221226505 / 0xC0000409)**: whisper-cli собран с ANSI CRT и крашится на путях с кириллицей (например, `C:\Users\Дамир\...` — символы превращаются в `?????`, модель не открывается → `abort()`). Распознавание теперь выполняется из ASCII-рабочей папки: модель и аудио копируются в неё (модель кэшируется, переиспользуется), результат читается оттуда же.
